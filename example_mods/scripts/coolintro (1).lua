tweens = {
	...
	dad = function(i, isPlayer, strum, set, get, tween)
		...
		tween({y = get 'y' - 920*d}, 0.5, {startDelay = 0.5 + (0.2 * i), ease = 'quartIn', onComplete = function()
	...
	bf = function(i, isPlayer, strum, set, get, tween)
		set('x', get 'x' + 550)
		set('angle', 180)
		tween({angle = 0, x = get 'x' - 550}, 1, {startDelay = 0.5 + (0.2 * i), ease = 'sineIn'})
	end
}
tweenNum = 98
function onStartCountdown()
	setProperty('skipArrowStartTween', false)
	luaDebugMode = true
end
tweenStarts = {}
tweenEnds = {}
...
function _tweenUpdate(t, p) if tweenUpdates[t] then tweenUpdates[t](p) end 
end